  ...
  <div class="app-container">
    ...
    <el-form :inline="true" class="demo-form-inline">
      ...
      <el-form-item label="出售状态">
        <el-select v-model="dataVo.status" clearable placeholder="请选择出售状态">
    ...
    <el-table
      ...
      <el-table-column prop="details" label="物品描述"/>
      <el-table-column prop="phone" label="联系方式"/>
      <el-table-column prop="price" label="出售价格"/>
      <el-table-column prop="userId" label="发布人">
        <template slot-scope="scope">
          <el-tag  v-for="item in userList" v-if="scope.row.userId == item.id">{{item.name}}</el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="status" label="出售状态">
        <template slot-scope="scope">
          <el-tag type="danger" v-if="scope.row.status == '0'">未出售</el-tag>
          <el-tag  v-if="scope.row.status == '1'">已出售</el-tag>
        </template>
      </el-table-column>
    ...
    <el-dialog
      ...
      <el-form :model="formData" label-position="left" label-width="100px">
        ...
        <el-form-item label="物品描述">
          <el-input v-model="formData.details" placeholder="请输入物品描述"/>
        </el-form-item>
        <el-form-item label="联系方式">
        ...
        <el-form-item label="出售状态">
          <el-select v-model="formData.status" placeholder="请选择出售状态">
            <el-option label="未出售" value="0"></el-option>
            <el-option label="已出售" value="1"></el-option>
          </el-select>
        </el-form-item>
...
<script>
  ...
  import secondhand from "@/api/secondhand";
  export default {
    data() { // 定义变量和初始值
      return {
    ...
    methods: { // 创建具体的方法
      ...
      getList(page = 1) {
        ...
        secondhand.pageQuery(this.page, this.limit, this.dataVo)
          .then(response => {// 请求成功
      ...
      handleBtn(){
        ...
        secondhand.saveTeacher(this.formData)
          .then(response => {// 请求成功
            this.$message({
              type: 'success',
              message: '操作成功！'
